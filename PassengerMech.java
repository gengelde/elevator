...
/**
 * The PassengerMech class represents the mechanism for managing passenger elevators.
 * It implements the Mechable, Elevatable, and Floorable interfaces.
 */
public class PassengerMech extends Elevator implements Mechable, Elevatable, Floorable
{
    private final int numPassengerElevators = 2;
    ...
    private HashMap<Integer, Elevator> passengerMap = new HashMap<>();
    ...
    /**
     * Constructs a new PassengerMech object and initializes the passenger elevators and floors.
     */
    public PassengerMech()
    {
        Factor = new Factory();
    ...
    /**
     * Initializes the passenger elevator and floor maps.
     */
    public void loadMaps()
    {
        for (int i = 0; i < numPassengerElevators; i++)
        {
            Elevator passenger = new Elevator();
            passengerMap.put(i, passenger);
        }
    ...
    /**
     * Checks if any passenger elevator has pending floor requests in its queue.
     *
     * @return {@code true} if at least one passenger elevator has pending floor requests,
    ...
    public boolean hasQueues()
    {
        for (int i = 0; i < numPassengerElevators; i++)
        {
            selectedElevator = passengerMap.get(i);
    ...
    /**
     * Retrieves the list of passenger elevators that have pending floor requests in their queues.
     *
     * @return An ArrayList containing the indices of passenger elevators with pending floor requests.
     */
    ...
    {
        for (int i = 0; i < passengerMap.size(); i++)
        {
            selectedElevator = passengerMap.get(i);
    ...
    public void processQueues(ArrayList<Integer> list, String dir)
    {
        ...
        {
            selectedElevator = passengerMap.get(list.get(i));
            processElevator(dir);
        }
        try
        {
    ...
    public void processElevator(String dir)
    {
        if (selectedElevator.getCurrentFloor() == selectedElevator.getQueue().getFirst() && dir.equals("EL"))
        {
            ...
            System.out.println("Passenger Elevator #" + selectedElevator.hashCode() + "\tStopped at Floor: " + selectedElevator.getQueue().getFirst() + "\tOccupancy: " + selectedElevator.getOccupants());
    ...
    public void moveElevator()
    {
        ...
        if (selectedElevator.getDirectionHeaded().equals("UP"))
        {
            ...
            System.out.print("\nPassenger Elevator #" + selectedElevator.hashCode() + "\tCurrent Floor: " + selectedElevator.getCurrentFloor() + "\tCurrent Floor Call: " + currentCall);
            System.out.println("\tDirection Moving: " + selectedElevator.getDirectionHeaded() + "\tOccupancy: " + selectedElevator.getOccupants());
